...
const getAuthHeaders = () => ({
  headers: {
    Authorization: `Bearer ${localStorage.getItem("deliveryPartnerToken")}`,
  },
});
...
export default function DeliveryDashboard() {
  ...
  // send location to the server every 10 seconds if tracking is enabled
  useEffect(() => {
    ...
    const sendLocation = (pos: GeolocationPosition) => {
      ...
      activeOrders.forEach((order) => {
        axios
          .put(
            `${API_URL}/delivery/${order.id}/location`,
            { lat, lng },
            getAuthHeaders(),
          )
          .catch((error) => {});
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/delivery/${otpModal}/complete`,
        { otp },
        getAuthHeaders(),
      );
  ...
  const handleCancel = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/delivery/${cancelModal}/cancel`,
        { reason: cancelReason },
        getAuthHeaders(),
      );
